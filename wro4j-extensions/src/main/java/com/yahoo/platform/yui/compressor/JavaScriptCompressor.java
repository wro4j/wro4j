/*
 * YUI Compressor Author: Julien Lecomte - http://www.julienlecomte.net/ Copyright (c) 2009 Yahoo! Inc. All rights
 * reserved. The copyrights embodied in the content of this file are licensed by Yahoo! Inc. under the BSD (revised)
 * open source license.
 */
...
public class JavaScriptCompressor {
  ...
  /*
   * Simple check to see whether a string is a valid identifier name. If a string matches this pattern, it means it IS a
   * valid identifier name. If a string doesn't match it, it does not necessarily mean it is not a valid identifier
   * name.
   */
  ...
  private static void optimizeObjLitMemberDecl(final ArrayList tokens) {
    ...
    for (i = 0, length = tokens.size(); i < length; i++) {
      if (((JavaScriptToken) tokens.get(i)).getType() == Token.OBJECTLIT && i > 0
          && ((JavaScriptToken) tokens.get(i - 1)).getType() == Token.STRING) {
        token = (JavaScriptToken) tokens.get(i - 1);
        tv = token.getValue();
        tv = tv.substring(1, tv.length() - 1);
  ...
  public JavaScriptCompressor(final Reader in, final ErrorReporter reporter) throws IOException, EvaluatorException {
  ...
  /*
   * Returns the identifier for the specified symbol defined in the specified scope or in any scope above it. Returns
   * null if this symbol does not have a corresponding identifier.
   */
  ...
  /*
   * If either 'eval' or 'with' is used in a local scope, we must make sure that all containing local scopes don't get
   * munged. Otherwise, the obfuscation would potentially introduce bugs.
   */
  ...
  private void parseExpression() {
    ...
    while (offset < length) {
      ...
      switch (token.getType()) {
        ...
        // warn("Using JScript conditional comments is not recommended." + (munge ?
        // " Moreover, using JScript conditional comments reduces the level of compression!" : ""), true);
        // }
        // break;
        ...
        case Token.NAME:
          ...
          if (mode == BUILDING_SYMBOL_TREE) {
            ...
            if (symbol.equals("eval")) {
              ...
              protectScopeFromObfuscation(currentScope);
              warn("Using 'eval' is not recommended."
                  + (munge ? " Moreover, using 'eval' reduces the level of compression!" : ""), true);

            }

          } else if (mode == CHECKING_SYMBOL_TREE) {

            if ((offset < 2 || (getToken(-2).getType() != Token.DOT && getToken(-2).getType() != Token.GET && getToken(
                -2).getType() != Token.SET))
      ...
      switch (token.getType()) {
        ...
        case Token.WITH:
          if (mode == BUILDING_SYMBOL_TREE) {
            ...
            warn("Using 'with' is not recommended."
                + (munge ? " Moreover, using 'with' reduces the level of compression!" : ""), true);
          }
          break;
  ...
  private StringBuffer printSymbolTree(final int linebreakpos, final boolean preserveAllSemiColons)
    ...
    while (offset < length) {
      ...
      switch (token.getType()) {
        ...
        case Token.SUB:
          ...
          if (offset < length) {
            token = getToken(0);
            if (token.getType() == Token.INC || token.getType() == Token.DEC || token.getType() == Token.ADD
    ...
    if (!preserveAllSemiColons &&  getToken(-1).getType() != Token.COMMENT) {
      // &&
