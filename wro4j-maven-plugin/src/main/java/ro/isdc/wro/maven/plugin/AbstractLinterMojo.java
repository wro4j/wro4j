...
/**
 * <p>
 * Contains common behavior for mojos responsible for static code analysis,
 * example: csslint, jslint, jshint.
 * </p>
 *
 * <p>
 * Type {@code <T>} indicates the type of lint errors reported by this mojo.
 * </p>
 *
...
public abstract class AbstractLinterMojo<T> extends AbstractSingleProcessorMojo {
	...
	/**
	 * Allows build to fail when the first error is encountered without searching
	 * for next errors. This flag is true by default. When interested in all errors,
	...
	/**
	 * Counts maximum acceptable number of errors, useful for progressive code
	...
	/**
	 * Contains errors found during jslint processing which will be reported
	 * eventually.
	 */
	...
	/**
	 * @return an instance of {@link ReportXmlFormatter} responsible for generating
	 *         lint report.
	 */
	protected abstract ReportXmlFormatter createXmlFormatter(LintReport<T> lintReport, FormatterType type);

	/**
	 * A method which should be invoked on each new resource processing, having as a
	 * side effect an increment of the counter holding the number of total processed
	 * resources.
	 */
	...
	/**
	 * @return true if the build can fail due to found errors based on existing
	 *         configuration.
	 */
