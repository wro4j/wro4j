...
public class TokenMapper {
  ...
  /**
   * Map of each Function node and all the variables in its current function scope, other variables found while
   * traversing the prototype chain and variables found in the top-level scope.
   */
  private final List functionVarMappings = new ArrayList();
  private final Map debugDataList = new HashMap();
  ...
  public TokenMapper(final ScriptNode parseTree) {
   ...
   */
  private String getMappedToken(final String token, final boolean hasNewMapping) {
    String newToken = null;
    Map tokens = null;
    final String blank = new String("");
    final int localScope = functionBracePositions.size() - 1;

    final String oldToken = getPreviousTokenMapping(token, hasNewMapping);
   ...
   * @param token
   *          value of the string token
   * @return boolean value indicating if the token is present in the chained scope
   */
  private boolean isInScopeChain(final String token) {
    final int scope = functionBracePositions.size();
    final Map chainedScopeVars = (Map) functionVarMappings.get(functionNum);
    if (!chainedScopeVars.isEmpty()) {
      for (int i = scope; i > 0; i--) {
        if (chainedScopeVars.containsKey(new Integer(i))) {
          parentScope = i - 1;
          final List temp = Arrays.asList((String[]) chainedScopeVars.get(new Integer(i)));
   ...
   */
  private String getPreviousTokenMapping(final String token, final boolean hasNewMapping) {
    String result = new String("");
    final int scope = replacedTokens.size() - 1;
    ...
    if (hasNewMapping) {
      final Map tokens = (Map) (replacedTokens.get(scope));
      ...
      for (int i = scope; i > -1; i--) {
        final Map tokens = (Map) (replacedTokens.get(i));
        if (tokens.containsKey(token)) {
          result = (String) tokens.get(token);
          return result;
        }
   ...
   */
  private void collectFunctionMappings(final ScriptNode parseTree) {
    final int level = -1;
   ...
   *          scoping level
   */
  private void collectFuncNodes(final ScriptNode parseTree, int level, final ScriptNode parent) {
    level++;
    ...
    final int nestedCount = parseTree.getFunctionCount();
   ...
   * @param parseTree
   *          Mapping of each function node and corresponding parameters & variables names
   * @return compressed script
   */
  public int sourceCompress(final String encodedSource, int offset, final boolean asQuotedString,
      final StringBuffer sb, final int prevToken, final boolean inArgsList, final int currentLevel,
      final ReplacedTokens replacedTokens) {
    ...
    if (sb != null) {
      ...
      if (((functionBracePositions.size() > 0) && (currentLevel >= (((Integer) functionBracePositions.get(functionBracePositions.size() - 1)).intValue())))
          || (inArgsList)) {
    ...
    } else if (((functionBracePositions.size() > 0) && (currentLevel >= (((Integer) functionBracePositions.get(functionBracePositions.size() - 1)).intValue())))
        || (inArgsList)) {
      if (prevToken != Token.DOT) {
  ...
  public boolean leaveNestingLevel(final int braceNesting) {
    boolean tokensRemoved = false;
    final Integer bn = new Integer(braceNesting);

    if ((functionBracePositions.contains(bn)) && (replacedTokens.size() > 0)) {
      // remove our mappings now!
      final int scopedSize = replacedTokens.size();
  ...
  public DebugData getDebugData(final Integer functionPosition) {
