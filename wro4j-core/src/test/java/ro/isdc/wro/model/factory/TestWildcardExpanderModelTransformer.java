...
public class TestWildcardExpanderModelTransformer {
  ...
  public void wildcardResourcesAreOrderedAlphabetically() {
    final WroModel model = new WroModel();
    final String uri = String.format(ClasspathUriLocator.PREFIX + "%s/expander/order1/**.js", WroUtil.toPackageAsFolder(getClass()));
    model.addGroup(new Group("group").addResource(Resource.create(uri, ResourceType.JS)));
    Mockito.when(decoratedFactory.create()).thenReturn(model);
    ...
    final String resultPathPrefix = String.format("%s%s/expander/order", ClasspathUriLocator.PREFIX, WroUtil.toPackageAsFolder(getClass()));
