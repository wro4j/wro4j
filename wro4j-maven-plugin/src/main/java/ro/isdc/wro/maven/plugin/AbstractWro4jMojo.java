...
public abstract class AbstractWro4jMojo extends AbstractMojo {
	 ...
	 */
	@Parameter(defaultValue = "${basedir}/src/main/webapp/WEB-INF/wro.xml")
	...
	/**
	 * The folder where web application context resides useful for locating
	 * resources relative to servletContext. It is possible to provide multiple
	 * context folders using a CSV. When multiple contextFolders are provided, the
	 * servletContext locator will try to search in next contextFolder when a
	 * resource could not be located. By default, a single context folder is
	 * configured.
	 */
	...
	/**
	 * Comma separated group names. This field is optional. If no value is provided,
	 * a file for each group will be created.
	 */
	@Parameter
	 ...
	 */
	@Parameter(defaultValue = "${basedir}/src/main/webapp/WEB-INF/wro.properties")
	private File extraConfigFile;

	/**
	 * Responsible for identifying the resources changed during incremental build.
	 * <p/>
	 * Read more about it <a href=
	 * "http://wiki.eclipse.org/M2E_compatible_maven_plugins#BuildContext">here</a>
	 */
	@Component
	private BuildContext buildContext;

	/**
	 * This parameter is not meant to be used. The only purpose is to hold project
	 * build directory
	 */
	...
	/**
	 * Flag which allows to enable incremental build (experimental feature). It is
	 * false by default, but probably can be changed to true if no unexpected
		...
		if (skip) {
			...
			try {
				...
				final String message = "Exception occured while processing: " + e.toString() + ", class: "
						+ e.getClass().getName() + ",caused by: "
						+ (e.getCause() != null ? e.getCause().getClass().getName() : "");
				getLog().error(message, e);
				if (e instanceof WroRuntimeException) {
					// Do not keep resources which cause the exception. This is helpful for linter
					// processors.
	...
	/**
	 * Safely invoke {@link ResourceChangeHandler#forget(Resource)}. The safety is
	 * required because invoking {@link #getResourceChangeHandler()} can throw an
	...
	/**
	 * Creates a custom instance of Manager factory. The wroManagerFactory parameter
	 * value is used to identify the manager class.
	 */
	private WroManagerFactory createCustomManagerFactory() throws MojoExecutionException {
		WroManagerFactory factory = null;
		try {
			final Class<?> wroManagerFactoryClass = Thread.currentThread().getContextClassLoader()
					.loadClass(wroManagerFactory.trim());
	...
	/**
	 * Allows explicitly to override the default implementation of the factory,
	 * assuming {@link #wroManagerFactory} configuration is not set. When overriding
	 * this method, make sure that {@link #onAfterCreate(WroManagerFactory)} is
	...
	/**
	 * Initialize the created {@link WroManagerFactory} with additional
	 * configurations which are not available during creation. Make sure this method
	 * is invoked on a custom factory when {@link #newWroManagerFactory()} is
	 * overridden.
	 */
	private void onAfterCreate(final WroManagerFactory factory) throws MojoExecutionException {
	...
	protected void validate() throws MojoExecutionException {
		if (wroFile == null) {
			throw new MojoExecutionException("The wroFile was not set!");
		}
		if (contextFolder == null) {
			throw new MojoExecutionException("No context folder was set!");
		}
	}
	...
	protected final void extendPluginClasspath() throws MojoExecutionException {
		...
		// http://teleal.org/weblog/Extending%20the%20Maven%20plugin%20classpath.html
	...
	/**
	 * @return The {@link TaskExecutor} responsible for running multiple tasks in
	 *         parallel.
	 */
	...
	/**
	 * @return true if the only incremental changed group should be used as target
	 *         groups for next processing.
	 */
	...
	/**
	 * Invoked right after execution completion. This method is invoked also if the
	...
	private ResourceChangeHandler getResourceChangeHandler() {
		if (resourceChangeHandler == null) {
			resourceChangeHandler = ResourceChangeHandler.create(getManagerFactory(), getLog())
					.setBuildContext(buildContext).setBuildDirectory(buildDirectory)
					.setIncrementalBuildEnabled(incrementalBuildEnabled);
		}
	...
	/**
	 * @param contextFolders a CSV representing contextFolders to use.
	 */
	...
	/**
	 * @param wroManagerFactory fully qualified name of the
	 *                          {@link WroManagerFactory} class.
	 */
