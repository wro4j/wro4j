...
public class ClasspathUriLocator
  ...
  private InputStream locateWildcardStream(final String uri, final String location)
    ...
    final Enumeration<URL> systemURLEnumeration = ClassLoader.getSystemResources(fullPath);
    final List<URL> systemUrlList = Collections.list(systemURLEnumeration);

    final Enumeration<URL> localURLEnumeration = Thread.currentThread().getContextClassLoader().getResources(fullPath);
    final List<URL> localUrlList = Collections.list(localURLEnumeration);

    final Enumeration<URL> classURLEnumeration = getClass().getClassLoader().getResources(fullPath);
    final List<URL> classUrlList = Collections.list(classURLEnumeration );
    System.out.println(localUrlList);
