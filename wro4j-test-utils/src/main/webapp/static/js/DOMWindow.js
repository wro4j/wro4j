(function($){
	...
	$.fn.closeDOMWindow = function(settings){
		...
		if(settings.functionCallOnClose){settings.functionCallBeforeClose();}
		...
		var run = function(passingThis){
			...
			if(settings.anchoredClassName){
				...
				$('#DOMWindow').fadeOut('fast',function(){
					//EZ:$('#DOMWindow').draggable("destroy").trigger("unload").remove();
	...
	$.fn.openDOMWindow = function(instanceSettings){	
		...
		var showDOMWindow = function(instance){
			if(arguments[0]){
				...
				$('.'+instance+' #DOMWindowContent').fadeIn('fast');
				$('.'+instance+ '.closeDOMWindow').click(function(){
					$.closeDOMWindow({close:true});	
					return false;
				});
			}else{
		...
		var run = function(passingThis){
			...
			if(settings.positionType == 'anchored'){//anchored DOM window
				...
				if(settings.functionCallOnOpen){settings.functionCallOnOpen();};
				...
				if(settings.overlay){
					...
					if(!settings.modal){ $('#DOMWindowOverlay').click(function(){$.closeDOMWindow({close:true})}) };
				};
