...
/**
 * Responsible to locate a context relative resource. Attempts to locate the
 * resource using {@link RequestDispatcher} . If the dispatcher fails, it will
 * fallback resource retrieval to a http call using {@link UrlUriLocator}.
 *
...
public class DispatcherStreamLocator {
	...
	/**
	 * /** When using JBoss Portal and it has some funny quirks...actually a portal
	 * application have several small web application behind it. So when it
	 * intercepts a requests for portal then it start bombing the the application
	 * behind the portal with multiple threads (web requests) that are combined with
	 * threads for wro4j.
	 *
	 * @param location a path to a request relative resource to locate.
	 * @return a valid stream for required location. This method will never return a
	 *         null.
	 * @throws IOException if the stream cannot be located at the specified
	 *                     location.
	 */
	 ...
	 * When original request was made through a proxy server, use
	 * "X-Forwarded-Proto" if present
	 *
	 ...
	 * @throws IOException
	 */
	public InputStream locateExternal(final HttpServletRequest request, final String location) throws IOException {
	...
	/**
	 * @return the {@link UriLocator} responsible for locating resources when
	 *         dispatcher fails. No wildcard handling is required.
	 * @VisibleForTesting
	 */
